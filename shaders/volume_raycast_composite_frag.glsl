...
void main()
{
	...
	while(true)
	{
		...
		vec4 transfer   = vec4(ambient + diffuse + specular, 1.0f) * texture(transferTexture, vec2(currIntensity, 0.0f));
		...
		// Early ray termination on arbitrary threshold
		if(outputColor.a >= 0.90)
			break;
